...
*/
/**************************************************************************/
void Adafruit_BMP085_Unified::RCR_requestRawPressure()
{
#if BMP085_USE_DATASHEET_VALS
	*pressure = 23843;
#else
...
{
	// Enable I2C
#if DEBUG_EMERGENCY && DEBUG_ALTITUDEPLZ
...
| | \ \| |____| | \ \  | |  | |_| | | | | (__| |_| | (_) | | | \__ \
|_|  \_\\_____|_|  \_\ |_|   \__,_|_| |_|\___|\__|_|\___/|_| |_|___/*/
/**************************************************************************/
/*!

*/
/**************************************************************************/
static void RCR_readRawPressure(int32_t *pressure) {
#if BMP085_USE_DATASHEET_VALS
	*pressure = 23843;
#else
	uint8_t  p8;
...
void Adafruit_BMP085_Unified::RCR_getEvent(sensors_event_t *event)
{
	float pressure_kPa;
...
*/
/**************************************************************************/
void Adafruit_BMP085_Unified::RCR_getPressure(float *pressure)
{
	int32_t  ut = 0, up = 0, compp = 0;
	int32_t  x1, x2, b5, b6, x3, b3, p;
	uint32_t b4, b7;
	...
	/* Get the raw pressure and temperature values */
	RCR_readRawPressure(&up);
	readRawTemperature(&ut);
	RCR_requestRawPressure();

#if DEBUG_EMERGENCY && DEBUG_ALTITUDEPLZ
	Serial.print("RCR_readRawPressure returned: ");
	Serial.print(up);
	Serial.print("  and readRawTemperature returned: ");
	Serial.print(ut);
	Serial.print(" at t = ");
...
*/
/**************************************************************************/
bool Adafruit_BMP085_Unified::RCR_readyYet() {
	uint8_t measCtrlReg;

	read8(BMP085_REGISTER_CONTROL, &measCtrlReg);
#if DEBUG_EMERGENCY && DEBUG_ALTITUDEPLZ
	Serial.print("The Measurement control reg reads: ");
	Serial.print(measCtrlReg);
	Serial.print(" at t = ");
	Serial.println(millis());
#endif
